...
std::vector<diversity_function> Diversity::strategies(std::string csl) {
    ...
    for(auto s: strategies) {
        ...
        if(s == "difference") {
            // Do nothing. The DFS engine will not explore the same combination twice so
...
void Diversity::registers(GlobalModel* home, const GlobalModel& b) {
    ...
    rel(*home, BOT_AND, equal_old, 1); // ^ across equal_old equal 0 (i.e not all equal_old[i] is true)
}
